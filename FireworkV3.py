...
import asyncio, pygame, random, math, concurrent.futures, numpy as np
...
def is_ui_window_active(manager):
    for element in get_all_ui_elements(manager.get_root_container()):
        if isinstance(element, pygame_gui.windows.UIColourPickerDialog):
            return True
    return False
...
class Firework:
    def __init__(self, pos, mode, color=None):
        self.pos = Vector2(pos)
        self.color = tuple(max(0, min(255, int(x))) for x in color[:3]) if color else (
            random.randint(128, 255),
            random.randint(128, 255),
            random.randint(128, 255)
        )
        self.rocket = Particle(self.pos, (random.uniform(-2, 2), random.uniform(-22, -18)),
                               self.color, 1.0, 2.2, drag=0.96, gravity=0.05)
        self.exploded = False
        self.particles = []
        self.mode = mode
        self.secondary = []
        ...
        #Random delay config; [SOLVED: minimal delay]
        self.secondary_delay = settings.secondary_explosion_delay + random.uniform(0.3, 1.0)
        self.second_mode = random.choice(settings.modes)
    ...
    def update(self, dt):
        if not self.exploded:
            ...
            if not self.second_exploded:
                ...
                if self.secondary_timer >= self.secondary_delay and len(self.particles) >= 15:
    ...
    def explode(self):
        if self.mode == "burst":
            for _ in range(60):
                ...
                self.particles.append(Particle(self.rocket.pos, (vx, vy),
                                               self.color, 1.3,
            ...
            for i in range(20):
                ...
                self.particles.append(Particle(self.rocket.pos, (vx, vy),
                                               self.color, 1.6,
                                               random.uniform(1, 2),
                                               drag=0.96, gravity=0.12))

    def secondary_explode(self, center):
        if self.second_mode == "burst":
            for _ in range(30):
                a = random.uniform(0, 2 * math.pi)
                ...
                self.secondary.append(Particle(center, (vx, vy), c,
                                               1.2, random.uniform(1, 2),
                                               0.95, 0.1))
        elif self.second_mode == "ring":
            for i in range(18):
                a = i * (2 * math.pi / 18)
                vx, vy = math.cos(a) * 5, math.sin(a) * 5
                c = (
                    min(255, self.color[0] + random.randint(-60, 60)),
                    min(255, self.color[1] + random.randint(-60, 60)),
                    min(255, self.color[2] + random.randint(-60, 60))
...
class CloudLayer:
    def __init__(self):
        ...
        self.tmp_surf = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
    def generate_noise_surf(self):
        w2, h2 = WIDTH // 2, HEIGHT // 2
        arr = np.random.randint(0, 81, (h2, w2), dtype=np.uint8)
        noise_array = np.empty((h2, w2, 4), dtype=np.uint8)
        noise_array[..., 0] = arr
        noise_array[..., 1] = arr
        noise_array[..., 2] = arr
        noise_array[..., 3] = 255
        surf = pygame.image.frombuffer(noise_array.tobytes(), (w2, h2), "RGBA")
    ...
    def draw(self, surf):
        ...
        self.tmp_surf.blit(self.noise_surf, (-ox, -oy))
        self.tmp_surf.blit(self.noise_surf, (-ox + WIDTH, -oy))
        self.tmp_surf.blit(self.noise_surf, (-ox, -oy + HEIGHT))
        self.tmp_surf.blit(self.noise_surf, (-ox + WIDTH, -oy + HEIGHT))
        self.tmp_surf.set_alpha(self.alpha)
        surf.blit(self.tmp_surf, (0, 0), special_flags=pygame.BLEND_ALPHA_SDL2)
...
class Settings:
    def __init__(self):
        ...
        self.secondary_explosion_delay = 1
...
effect_mode_dropdown = pygame_gui.elements.UIDropDownMenu(
    options_list=settings.modes,
    starting_option=settings.effect_mode,
    relative_rect=pygame.Rect((10, 10), (150, 30)),
    manager=manager
)
toggle_color_button = pygame_gui.elements.UIButton(
...
async def main_loop():
    global launching, time_since_launch
    while True:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                for element in get_all_ui_elements(manager.get_root_container()):
                    if element.visible and element.combined_rect.collidepoint(event.pos):
                        ui_under_mouse = True
                        break
                if not ui_under_mouse and not is_ui_window_active(manager):
                    launching = True
            ...
            if event.type == pygame_gui.UI_BUTTON_PRESSED:
                ...
                if event.ui_element == pick_color_button:
                    pygame_gui.windows.UIColourPickerDialog(
                        ...
                        initial_colour=pygame.Color(*settings.custom_color)
                    )
